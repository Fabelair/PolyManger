...
function Ajouter(){
    ...
    const [showIngredientsState, setShowIngredientState] = useState(true);
      ...
      <div class="w-[100%] h-flex">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div class="w-[32rem] h-flex rounded-2xl mt-4 shadow-xl ring ring-slate-700 place-self-center grid">
            ...
            <div class="w-[80%] ml-[10%] h-flex mb-4">
              <div class="grid grid-cols-10">
                ...
                <label for="message" class="block mb-2 ml-2 text-md font-medium text-gray-900 dark:text-white col-span-9">Ingrédients :</label>
              </div>
              {showIngredientsState && <>
                <div class=" h-flex grid grid-cols-3">
                  <label for="message" class="block mb-2 ml-16 col-span-2 text-md font-medium text-gray-900 dark:text-white">Nom</label>
                  <label for="message" class="block mb-2 ml-8 text-md font-medium text-gray-900 dark:text-white">Quantité</label>
                </div>
                <div class=" h-flex mt-2 mb-4">
                  {ingredients.map((ingredient,index) => 
                    <div class=" h-10 mb-4 grid grid-cols-9"> 
                      <div class=" grid">
                        <h1 class="text-xl dark:text-white font-bold text-center place-self-center">{index+1}.</h1>
                      </div>
                      <div class=" col-span-5 ml-2 mr-2">
                        <input type="text" class="h-full w-full dark:bg-gray-900 dark:text-white text-bold p-2 border border-b-white border-t-transparent border-l-transparent border-r-transparent"></input>
                      </div>
                      <div class=" col-span-3 ml-4">
                        <input type="text" class="text-center h-full w-full dark:bg-gray-900 dark:text-white text-bold p-2 border border-b-white border-t-transparent border-l-transparent border-r-transparent"></input>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div class="w-[32rem] h-flex mt-8 place-self-center grid">
            <button onClick={()=>{submit()}} class="h-12 rounded-lg w-[50%] place-self-center text-2xl text-center font-bold bg-blue-700 text-white">Submit</button>
          </div>
        </form>
      </div>
    </div>
